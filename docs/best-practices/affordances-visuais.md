...
- Qualquer elemento clicável (barra, item de lista, botão de controle) usa
  `cursor: pointer`.
- Elementos arrastáveis usam `cursor: grab` em repouso e `cursor: grabbing`
  durante o arraste — nunca o cursor padrão de texto/seta.
...
- Todo alvo clicável muda de estado no hover (fundo, borda ou cor do
  texto) antes do clique confirmar a ação.
- O hover não pode ser a única pista em telas touch — combine com formato
  de botão/borda que já sugere clicabilidade mesmo sem hover.

## 3. Estado atual visível

- Controles com estado (toggle, métrica selecionada, item selecionado)
  precisam parecer visualmente diferentes de controles inativos — cor de
  destaque (`accent`), borda ou peso de fonte.
...
- Nunca estilize um elemento como se fosse clicável se ele não faz nada —
  isso quebra a confiança do usuário no resto do dashboard.
- O inverso também vale: se um elemento é clicável mas parece estático, o
  usuário nunca vai descobrir.
